...
function saveData() {
    try {
        ...
        // Get the active manual savings allocation value to protect it from disappearing from assets
        let activeManualSavings = (financialData.customSavingsOverride !== null && financialData.customSavingsOverride !== undefined) ? Number(financialData.customSavingsOverride) : 0;

        // PERFECTED LOGIC FIX: Both active savings goals AND manual savings pool overrides are added back.
        // This ensures moving funds to savings reduces Current Balance but keeps Total Net Assets completely unchanged.
        absoluteGrandTotalWorth = financialData.balance + totalGoalFundsAllocated + totalInvestmentsAccumulated + activeManualSavings + externalWalletsSum;
    ...
    // 4. TRANSACTION MODULE FEED VIEW (DUAL DATE & TIME VIEW SYNC MAINTAINED)
        ...
        if (Array.isArray(financialData.transactions)) {
            financialData.transactions.slice().reverse().forEach(tx => {
                const isIncome = tx.type === 'income' || tx.type === 'monthly_pool' || (tx.category && tx.category.toLowerCase() === 'salary');
            ...
            <div style="padding: 20px; padding-bottom: 95px;">
                ...
                <div class="stats-grid" style="display:grid; grid-template-columns:1fr 1fr; gap:12px; margin-bottom:15px;">
                    <div class="stat-card" style="background:var(--card-bg); border:1px solid var(--border-color); padding:12px; border-radius:14px;">
                        <p style="margin:0; font-size:11px; color:var(--text-muted);">Total Expenses</p>
                        <h4 style="margin:5px 0 0 0; color:var(--danger-red); font-size:15px;">Rs. ${displayExpense.toLocaleString()}</h4>
                    </div>
                    <div class="stat-card" style="background:var(--card-bg); border:1px solid var(--border-color); padding:12px; border-radius:14px;">
...
window.openBackupManagementModal = function () {
    ...
    const totalNetWorth = absoluteGrandTotalWorth;
    createModalOverlay(`
...
// ==========================================
// 📥 SECURE BACKUP DATA IMPORT ENGINE LOGIC
// ==========================================
    ...
    reader.onload = function (e) {
        try {
            ...
            if (parsedData && typeof parsedData === 'object') {
                ...
                financialData.userStatus = parsedData.userStatus !== undefined ? parsedData.userStatus : financialData.userStatus;
                financialData.bestLine = parsedData.bestLine !== undefined ? parsedData.bestLine : financialData.bestLine;
                financialData.initialSeedWallet = parsedData.initialSeedWallet !== undefined ? parsedData.initialSeedWallet : financialData.initialSeedWallet;
                financialData.previousSeedWallet = parsedData.previousSeedWallet !== undefined ? parsedData.previousSeedWallet : financialData.previousSeedWallet;
                ...
                financialData.investments = Array.isArray(parsedData.investments) ? parsedData.investments : [];
                financialData.wallets = Array.isArray(parsedData.wallets) ? parsedData.wallets : [];
                financialData.chatHistory = Array.isArray(parsedData.chatHistory) ? parsedData.chatHistory : financialData.chatHistory;

                // Sync data metrics to localStorage and recalculate Net Worth
                saveData();
                closeModal();
                switchScreen('dashboard');

                // Confirmation feedback screen reload trigger
                showGoalConfirmationToast("Backup file successfully imported & synchronized!");
            } else {
                alert("🚨 Error: Selected backup file structure is invalid.");
            }
        } catch (err) {
            console.error("Critical JSON Backup Extraction Interrupted:", err);
            alert("🚨 Error: Corrupted or mismatched JSON configuration file parsed.");
        }
    };
...
window.generateProfessionalPDFReceipt = function () {
    ...
    // Sync variables perfectly with the dashboard engine mapping
    const userName = financialData.user ? financialData.user : "Active CFO User";
    ...
    const savingsAmount = financialData.savingsAmount || 0;
    const savingsRate = financialData.savingsRate || 0;

    // Use absolute grand net worth variable directly from core dashboard calculations
    const calculatedNetWorth = typeof absoluteGrandTotalWorth !== 'undefined' ? absoluteGrandTotalWorth : curBalance;
    ...
    doc.setFont("Helvetica", "bold");
    doc.setTextColor(156, 39, 176); // Purple color for Savings Pool to isolate from Net Worth
    doc.text(`TOTAL SAVINGS POOL: Rs. ${Number(savingsAmount).toLocaleString()} (${savingsRate}%)`, 110, y);
    ...
    if (goalsList.length === 0) {
        ...
        goalsList.forEach(g => {
            ...
            // Using g.saved as specified in dashboard structure logic
            doc.text(`Rs. ${Number(g.saved || 0).toLocaleString()}`, 155, y);
            y += 6;
        });
    }
    ...
    // --- 6. SYSTEM TRANSACTION HISTORY LOGS (FIXED OVERRIDE OVERLAP) ---
    y += 10;
    ...
    doc.text("Description / Title", 55, y);
    ...
    if (transactionsList.length === 0) {
        ...
        transactionsList.slice().reverse().forEach((tx) => {
            ...
            doc.text(`${tx.title ? tx.title.substring(0, 20) : 'N/A'}`, 55, y);
            doc.text(`${tx.category || 'General'}`, 105, y);
            doc.text(`${(tx.type || '').toUpperCase()}`, 140, y);
            doc.text(`Rs. ${Number(tx.amount || 0).toLocaleString()}`, 170, y);
            y += 6.5;
        });
    }
